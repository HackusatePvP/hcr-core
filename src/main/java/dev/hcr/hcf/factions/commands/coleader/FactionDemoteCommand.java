...
public class FactionDemoteCommand extends FactionCommand {
    ...
    public FactionDemoteCommand() {
        super("demote", "Demote a member in the faction to the previous rank.");
    }
    ...
    public void execute(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!user.hasFaction()) {
            player.sendMessage(ChatColor.RED + "You must be in a faction to demote players.");
            return;
        }
        PlayerFaction playerFaction = (PlayerFaction) user.getFaction();
        if (playerFaction.getRole(user).getWeight() < 3) {
            player.sendMessage(ChatColor.RED + "Only co-leaders and above can demote players.");
            return;
        }
        if (args.length == 2) {
            String name = args[1];
            User userTarget = User.getUser(name);
            if (userTarget == null) {
                player.sendMessage(ChatColor.RED + "Could not find user \"" + name + "\".");
                return;
            }
            ...
            playerFaction.demoteUser(player, userTarget);
        }
    }
